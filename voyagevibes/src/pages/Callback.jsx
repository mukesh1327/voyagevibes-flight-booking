...
import { handleCallback, getCurrentUser, readOAuthCallbackError, storeTokens } from '../services/auth';
  ...
  useEffect(() => {
    const processCallback = async () => {
      ...
      try {
        ...
        if (userData.baseRole === 'corporate') {
          navigate('/corporate/dashboard');
        } else {
          // Check if first time user (we might need a real API for this, but for now we'll mock it or base it on something)
          // Since the prompt mentions "display details for first time users and for existing users just go to booking page"
          // We can just redirect to customer dashboard which handles the logic, or redirect to /booking directly
